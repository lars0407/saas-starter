...
export default function ResumePage() {
  ...
  const fetchDocuments = async (variant: string, page: number) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://api.jobjaeger.de/api:SiRHLF4Y/documents?offset=${offset}&variant=${variant}&type=resume`,
        {
          headers: {
      ...
      console.log("Resumes API response:", data) // Debug log
      ...
      console.log("Processed resumes:", documents)
  ...
  const handleView = (id: number) => {
    ...
    if (!document) {
      toast.error("Lebenslauf nicht gefunden 😅")
      return
    }
        ...
        <div className="flex items-center gap-2 px-2 sm:px-4">
          ...
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator className="hidden md:block" />
              <BreadcrumbItem>
                <BreadcrumbPage>Lebenslauf</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>
      ...
      <div className="flex flex-1 flex-col gap-4 p-2 sm:p-4 pt-0">
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div className="flex-1">
              <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">
                Deine Lebensläufe – time to shine ✨
              </h1>
              <p className="text-sm sm:text-base text-gray-600 mt-2">
                Hier findest du alle Lebensläufe, die du erstellt oder mit KI generiert hast.
              </p>
            </div>
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
          <TabsList className="grid w-full sm:w-fit grid-cols-2">
            <TabsTrigger value="human" className="text-xs sm:text-sm px-2 sm:px-3">Basis</TabsTrigger>
            <TabsTrigger value="ai" className="text-xs sm:text-sm px-2 sm:px-3">KI-generiert</TabsTrigger>
          </TabsList>
              ...
              <div className="flex items-center justify-center h-[calc(100vh-300px)] w-full">
                <div className="text-center">
                  ...
                  <h3 className="text-lg font-semibold text-gray-900 mb-2">
                    Noch kein Lebenslauf erstellt 😅
                  </h3>
                  <p className="text-gray-600 mb-6 max-w-md">
                    Fang jetzt an und erstelle deinen CV – dein Future-Ich wird's dir danken.
                  </p>
                  <Button 
                    onClick={handleCreateResume}
              ...
              <>
                ...
                <div className="mt-6 sm:mt-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <p className="text-xs sm:text-sm text-gray-500 whitespace-nowrap">
                    {totalDocuments} Lebenslauf{totalDocuments !== 1 ? 'e' : ''} gefunden
                  </p>
                  {renderPagination()}
                </div>
              </>
